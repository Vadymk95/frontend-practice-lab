...
- [ ] Task 1: Update `package.json` (AC: #1)
  - [ ] Change `"vite": "npm:rolldown-vite@7.3.1"` → `"vite": "^8.0.0"`
  - [ ] Check if `@vitejs/plugin-react` needs an update for Vite 8 compat
  - [ ] Check if `@tailwindcss/vite` supports Vite 8 (it should — uses Vite plugin API)
  - [ ] Check `vite-plugin-eslint2`, `vite-plugin-svgr`, `vite-plugin-compression`, `vite-plugin-webfont-dl` peer deps
  - [ ] Run `npm install`

- [ ] Task 2: Validate `vite.config.ts` compatibility (AC: #2, #3)
  - [ ] Read Vite 8 migration guide for breaking changes
  - [ ] Update any deprecated API calls if found
  - [ ] Verify `build.minify: 'oxc'` is still valid (OXC minifier was added in Vite 6 — should be fine)
  - [ ] Verify `build.target: 'baseline-widely-available'` is still supported

- [ ] Task 3: Validate custom plugins (AC: #2)
  - [ ] Check `./vite-plugins/html-optimize.ts` compatibility with Vite 8 plugin API
  - [ ] Check `./vite-plugins/i18n-hmr.ts` compatibility with Vite 8 plugin API
  - [ ] Run `npm run dev` — verify dev server + HMR work

- [ ] Task 4: Run full CI pipeline locally (AC: #4)
  - [ ] `npm run lint`
  - [ ] `npx tsc --noEmit`
  - [ ] `npm run validate:data` (if script exists)
  - [ ] `npm run test`
  - [ ] `npm run build`

- [ ] Task 5: Record baseline bundle size (AC: #3)
  - [ ] Run `npm run build` and note chunk sizes from output
  - [ ] Confirm no chunk exceeds 600kb (configured warning limit)
