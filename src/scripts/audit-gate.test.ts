...
// Guards the audit gate's fail-CLOSED policy. A security gate that returns success when it
// cannot run is worse than no gate, so the "invalid payload" case is the load-bearing one.
//
// Fixtures are built inline on purpose: a test that reads the live audit-allowlist.json
// breaks every time an allowance is added or expires, which trains people to edit the
// test instead of the policy.
import { describe, expect, it } from 'vitest';

import { type AllowlistEntry, evaluateAudit } from './audit-gate.ts';
...
const allowance = (overrides: Partial<AllowlistEntry> = {}): AllowlistEntry => ({
    id: ID,
    expires: '2026-06-01',
    reason: 'fixture',
    upstream: 'fixture',
...
describe('evaluateAudit', () => {
    it('passes a clean audit with an empty allowlist', () => {
        expect(evaluateAudit(audit({}), [], NOW)).toMatchObject({ ok: true, auditFailed: false });
    });
